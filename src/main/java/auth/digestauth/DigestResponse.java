...
public class DigestResponse {
    ...
    public static DigestResponse calculateDigestResponse(List<HttpHeader> responseHttpHeaders, EasyHttpRequest request){
        DigestConfigBuilder digestConfigBuilder = new DigestConfigBuilder();

        responseHttpHeaders.stream().filter(header-> header.getKey().equals("WWW-Authenticate"))
                .findFirst()
                .ifPresent(wwwAuthHeader -> {
                    ...
                    if(digestPropertyValueMap.containsKey("stale")) {
                        System.out.println("stale: " + digestPropertyValueMap.get("stale"));
                        digestConfigBuilder.setStale(!((String) digestPropertyValueMap.get("stale")).equals("FALSE"));
                    }
